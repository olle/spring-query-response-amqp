...
observe such a system is very important. We provide a monitoring and
administration user interface for this purpose.
...
Or use the published container image and run in your cloud environment of
choice, or using Docker:

[subs="attributes"]
....
$ docker run -t -p 8080:8080 {qr-ui-container}
....

NOTE: For a more complete quick-start example with Docker, please see the
      {qr-ui-name} project {qr-ui-gh-link}[README].

:ghcr-io: https://ghcr.io

The {qr-ui-name} container images are published to the public {ghcr-io}[GitHub
Packages] registry, and can be found in the project {qr-ui-container-link}[packages] 
listing.

== Overview

The {qr-ui-name} makes good use of the AMQP broker messages and events, and 
also uses Query/Response, to consume and query for information about the live
nodes in the current system setup. It is available at http://localhost:8080
